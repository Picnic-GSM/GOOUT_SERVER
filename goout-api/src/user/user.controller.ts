...
export class LoginController {
    ...
    @Post()
    @ApiCreatedResponse({
        description: '로그인 성공 후 access token 발급 완료',
    })
    @ApiBadRequestResponse({
        description: '입력된 데이터가 일치하지 않을 경우',
    })
    ...
    async loginForTeacher(@Body() req: LoginForTeacherDto) {
        ...
        return {
            accessToken: await this.authService.issueTokenForTeacher(
                teacherObj,
            ),
        };
    }
...
export class StudentController {
    ...
    @ApiBadRequestResponse({
        description:
            '이메일 형식 오류 or 이메일 중복 오류 or 학년, 반, 번호 오류',
    })
