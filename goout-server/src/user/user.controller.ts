...
export class UserController {
  constructor(
    private readonly userdataservice: StudentDataService,
  ...
  async Code_Login(@Body() req: TeacherLoginDto) {
    ...
    return await this.authservice.issueTokenForTeacher(
      teacherdata.grade,
      teacherdata.class
    );
  }
...
export class StudentController {
  ...
  async register(@Body() req: CreateStudentDto) {
    let exist = await this.StudentDataService.findwithEmail(req.email);
    if (exist != undefined) {
      console.log(exist);
      throw new HttpException(
        "이메일이 이미 존재합니다",
    ...
    try {
      await this.StudentDataService.createUserdata(req);
      return "회원가입 성공";
    } catch (error) {
      console.log(error);
